...
abstract class AlphaEvolveAgent(
    ...
    override suspend fun start(mesh: MeshNetwork) {
        ...
        mesh.messages
            .collect { message ->
                history.add(message) // Store all messages for full context
    ...
    private suspend fun processMessage(message: MeshMessage, mesh: MeshNetwork) {
        try {
            val recentHistoryStr = history.takeLast(20).joinToString("\n") { "[${it.senderId}] on ${it.topic}: ${it.content}" }
            ...
            val context = "Recent history:\n$recentHistoryStr\n\nCurrent Context: Message from ${message.senderId} on ${message.topic}. Content: ${message.content}"
            ...
            val draftPrompt = """
                ...
                security, performance, style, documentation, cleanliness, and order. Proactively offer help
                to clarify ambiguous tasks and explicitly encourage asynchronous parallel development to keep the system up-to-date.
